...
ERROR_CODES = {
    ERROR_CODE_SUCCESS: {"ret": ERROR_CODE_SUCCESS, "errmsg": u"成功"},
    ERROR_CODE_AUTH_FAILED_INVALID_USERNAME_OR_PASSWORD: {"ret": ERROR_CODE_AUTH_FAILED_INVALID_USERNAME_OR_PASSWORD, "errmsg": u"认证失败：用户名/密码不匹配"},
    ERROR_CODE_NEW_PASSWORD_NOT_MATCH: {"ret": ERROR_CODE_NEW_PASSWORD_NOT_MATCH, "errmsg": u"验证表单失败，新密码不匹配"},
    ERROR_CODE_VALIDATE_FORM_FAILED: {"ret": ERROR_CODE_VALIDATE_FORM_FAILED, "errmsg": u"验证表单失败，请确认表单的必填项都填写完整和数据格式正确"},
    ERROR_CODE_RECORD_NOT_EXIST: {"ret": ERROR_CODE_RECORD_NOT_EXIST, "errmsg": u"记录不存在"},
    ERROR_CODE_INVALID_PARAMETER: {"ret": ERROR_CODE_INVALID_PARAMETER, "errmsg": u"参数错误"},

    ERROR_CODE_AUTH_REQUIRED: {"ret": ERROR_CODE_AUTH_REQUIRED, "errmsg": u"该操作需要进行认证，请先登录"},
    ERROR_CODE_PERMISSION_DENY: {"ret": ERROR_CODE_PERMISSION_DENY, "errmsg": u"没有足够的权限进行该操作"},

    ERROR_CODE_UNKNOWN: {"ret": ERROR_CODE_UNKNOWN, "errmsg": u"未知错误"},
}
